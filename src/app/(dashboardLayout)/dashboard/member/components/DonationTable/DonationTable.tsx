...
export const DonationTable = () => {
  ...
  return (
    <div className="bg-white dark:bg-[#1a1716] p-4 md:p-6 rounded-2xl border border-[#efe9e6] dark:border-[#2f2824] shadow-sm transition-colors duration-300">
      {/* Filters - মোবাইল স্ক্রিনেও ভালো দেখাবে */}
          ...
          <thead>
            <tr className="text-gray-400 border-b border-gray-100 dark:border-[#2f2824]">
              {['RECEIPT', 'CAMPAIGN', 'AMOUNT', 'TYPE', 'DATE', 'STATUS', 'ACTION'].map((h, index) => (
                <th key={`${h}-${index}`} className="pb-4 px-2 font-normal">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100 dark:divide-[#2f2824]">
            {filteredData.map((d) => (
              <tr key={d.id} className="text-gray-900 dark:text-gray-200">
                ...
                <td className="py-4 px-2">{d.campaign}</td>
                <td className="py-4 px-2 font-bold">{d.amount}</td>
                <td className="py-4 px-2 text-gray-500">{d.type}</td>
                <td className="py-4 px-2 text-gray-500">{d.date}</td>
                <td className="py-4 px-2">
                  <span className="bg-[#eef2ef] dark:bg-[#1b2a21] text-[#4a7c59] dark:text-[#6ba97e] px-2 py-0.5 rounded text-xs">
                    {d.status}
                  </span>
                </td>
                <td className="py-4 px-2 text-[#009688] dark:text-[#6ba97e] flex items-center gap-1 cursor-pointer hover:underline">
