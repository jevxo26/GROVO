...
export const SupportedCampaigns: React.FC<SupportedCampaignsProps> = ({
  ...
  return (
    <div className="bg-card text-card-foreground p-6 rounded-3xl border border-border shadow-sm transition-all h-full flex flex-col justify-between">
      <div>
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Explore emergency appeals and verified community projects
            </p>
          </div>
          <Link
        ...
        {isGridMode ? (
          /* Grid View for standalone pages */
          ...
          /* List View for Dashboard Widget (Side-by-Side) */
          <div className="space-y-4">
            {campaigns.map((c, i) => (
              <div
                key={i}
                className="p-4 rounded-2xl border border-border/70 bg-card hover:bg-muted/20 hover:border-primary/40 transition-all duration-300 shadow-sm space-y-3 group"
              >
                {/* Header Tag & Title */}
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className="font-extrabold text-xs text-primary shrink-0 bg-primary/10 px-2 py-0.5 rounded-lg">
                    {c.percentage}%
                  </span>
                </div>

                {/* Progress Bar */}
                <div className="w-full bg-muted h-2 rounded-full overflow-hidden p-0.5">
                  <div
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs pt-1 border-t border-border/40">
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
                    <span className="font-semibold text-foreground">
                      ৳ {c.raised} <span className="text-muted-foreground font-normal">/ ৳ {c.target}</span>
                    </span>
                  ...
                  <Button
                    size="sm"
                    onClick={() => onDonateClick?.(c)}
                    className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl text-xs font-bold gap-1 px-3 self-end sm:self-auto"
                  >
                    Support <ArrowUpRight className="w-3.5 h-3.5" />
                  </Button>
                </div>
