...
const registerUser = catchAsync(async (req, res) => {
  const { firstName, lastName, email, phone, password, dateOfBirth, gender, bloodGroup, nationalId, address } = req.body;

  if (!firstName || !lastName || !email || !password) {
    throw new customError(httpStatus.BAD_REQUEST, "Required fields are missing.");
  }
...
const verifyOtp = catchAsync(async (req, res) => {
  ...
  if (!email || !otp) {
    throw new customError(httpStatus.BAD_REQUEST, "Email and OTP are required.");
  }
...
const rejectMembership = catchAsync(async (req, res) => {
  ...
  const result = await userService.rejectMembership(id, adminId, remarks || "No reason specified.");
...
const renewMembership = catchAsync(async (req, res) => {
  ...
  if (!amount) {
    throw new customError(httpStatus.BAD_REQUEST, "Renewal amount is required.");
  }

  const result = await userService.renewMembership(id, { amount, processedBy: adminId });
